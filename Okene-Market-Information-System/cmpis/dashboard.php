...
$priceQuery = "
    SELECT p.price, p.unit, p.date_recorded, c.commodity_name, m.market_name, 
           u.fullname as recorder, u.phone_number, u.role as recorder_role, u.id as recorder_id
    FROM prices p
    ...
    <aside class="w-64 bg-okene-green text-white flex flex-col justify-between hidden lg:flex shadow-xl z-20">
        <div>
            ...
            <nav class="p-4 space-y-1">
                ...
                <a href="chat.php" class="flex items-center space-x-3 text-green-100 hover:bg-white/5 px-4 py-2.5 rounded-lg text-sm transition-colors"><span>💬</span> <span>Messages</span></a>
                <a href="profile.php" class="flex items-center space-x-3 text-green-100 hover:bg-white/5 px-4 py-2.5 rounded-lg text-sm transition-colors"><span>👤</span> <span>My Profile</span></a>
                                ...
                                <?php else: ?>
                                    ...
                                    <tr class="hover:bg-gray-50/50 transition-colors">
                                        ...
                                        <td class="py-3.5 px-4 text-xs">
                                            <a href="view_profile.php?id=<?php echo $row['recorder_id']; ?>" class="text-okene-green hover:text-green-800 font-semibold underline decoration-green-200 underline-offset-2 transition-colors flex items-center space-x-1">
                                                <span><?php echo htmlspecialchars($row['recorder'] ?? 'System Seeder'); ?></span>
                                                <span class="text-gray-400 hover:text-okene-green text-[10px]">↗</span>
                                            </a>
                                        </td>
                                    </tr>
                                    <?php endforeach; ?>
...
<!-- AUTO LOGIN MESSAGE POP-UP NOTIFICATION (TOP CENTER) -->
<?php if (isset($_SESSION['show_welcome_message'])): ?>
    <div id="welcomeToast" class="fixed top-6 left-1/2 transform -translate-x-1/2 -translate-y-32 opacity-0 transition-all duration-700 ease-out z-50 w-full max-w-sm px-4">
        <div class="bg-white border-b-4 border-okene-green shadow-2xl rounded-xl p-4 flex items-start space-x-4">
            <div class="bg-green-100 p-2 rounded-full text-okene-green mt-1 shrink-0">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z"></path></svg>
            </div>
            <div class="flex-1">
                <h4 class="text-sm font-bold text-gray-800">Welcome to the Hub!</h4>
                <p class="text-xs text-gray-500 mt-1">Check your inbox to connect with local farmers and traders.</p>
                <div class="mt-3 flex space-x-3">
                    <a href="chat.php" class="text-xs font-bold text-white bg-okene-green hover:bg-green-800 px-3 py-1.5 rounded-md transition-colors shadow-sm">
                        Open Messages
                    </a>
                    <button onclick="document.getElementById('welcomeToast').classList.add('-translate-y-32', 'opacity-0')" class="text-xs font-medium text-gray-500 hover:text-gray-800">
                        Dismiss
                    </button>
                </div>
            </div>
    ...
    <script>
        // Trigger the drop-down animation 500ms after the dashboard loads
        setTimeout(() => {
    ...
    <?php unset($_SESSION['show_welcome_message']); ?>
<?php endif; ?>
