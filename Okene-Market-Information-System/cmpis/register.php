    ...
    if (!in_array($role, $allowed_roles)) {
        ...
        try {
            ...
            if ($checkStmt->rowCount() > 0) {
                ...
                $insertStmt = $pdo->prepare("INSERT INTO users (fullname, username, password, phone_number, role) VALUES (:fullname, :username, :password, :phone, :role)");
                $insertStmt->execute([
                    'fullname' => $fullname,
                    'username' => $username,
                    'password' => $hashedPassword,
                    'phone' => $_POST['phone_number'], // New field
                    'role' => $role
                ]);
    ...
    <div class="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md border border-gray-100">
        ...
        <form action="register.php" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <input type="tel" name="phone_number" class="w-full border border-gray-300 rounded-lg py-2 px-3 focus:outline-none focus:ring-2 focus:ring-okene-green" placeholder="e.g., 08012345678">
            </div>
